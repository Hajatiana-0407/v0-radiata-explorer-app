...
export function DestinationCard({ destination, link = true }: DestinationCardProps) {
  const content = (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white hover:shadow-2xl transition-all duration-300 group">
      <div
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-end justify-between pt-2">
          <div>
            <p className="text-xs text-slate-500 font-medium">À partir de</p>
            <p className="text-2xl font-bold" style={{ color: "#7ac243" }}>
              {destination.price}€
            </p>
          </div>
          <button
            className="px-4 py-2 rounded-lg font-semibold text-slate-900 transition-all hover:shadow-lg"
